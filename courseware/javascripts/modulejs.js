		...
		init: function(){
			...
			$('#btnNext').click(function(){
				...
				if(parseInt(les1)+1 <= lessonSize){
					...
					ipc.send('asynchronous-message', "\\module.html?mod="+(parseInt(mod1)+1)+"&les="+1);
		...
		getDecryptData: function(data){
    		...
    		for (var i=0;i<data.length;i++){
    			...
    			var cipher = crypto.createCipher(algorithm,password)
  				var crypted = cipher.update('friends = friends.replace(nonFriend.name + " ", "");','utf8','hex')
			...
			$.getJSON("moduledata.json", function(data){
				...
				if(data["modules"][mod1]["lessons"][parseInt(les1)-1]['programtype'] == 'graphic'){
					if(data["modules"][mod1]["lessons"][parseInt(les1)-1]['output'] == 'Testcase Passed: true\r<br>' || data["modules"][mod1]["lessons"][parseInt(les1)-1]['output'] == 'Testcase Passed: false\r<br>'){
						$('#result').html('<p>Actual Output: &nbsp;&nbsp;<img id="answer" src="program_images\\answer.png"></p>'+'<br><br><p>Expected Output: &nbsp;&nbsp;<img src="program_images\\'+data["modules"][mod1]["lessons"][parseInt(les1)-1]['classname']+'.png"></p>'+'<br><br>'+data["modules"][mod1]["lessons"][parseInt(les1)-1]['output']);
						if(data["modules"][mod1]["lessons"][parseInt(les1)-1]['output'] == 'Testcase Passed: true\r<br>'){
					...
					var apiQuestions = ["5.45","5.53","6.2","6.14","6.37","6.41","6.17"];
					var nonEvalQid = data["modules"][mod1]["lessons"][parseInt(les1)-1]['qid'];
					if($.inArray(nonEvalQid, apiQuestions) > 0){
						$('#result').html("Good Job. Your submission was unique and beautiful. However we do not know how to assess it at this moment. Please keep progressing.");
						$('#tipblock').html('<h4 style="color:green"><span class="glyphicon glyphicon-thumbs-up" aria-hidden="true"></span>&nbsp;Great Job!</h4>');
					}
					else{
			...
			var editor = ace.edit("editor");
				...
				$('#viewanswer').click(function(){
					...
					setInterval(function(){
    					$("#answer").attr("src", "program_images\\answer.png?"+new Date().getTime());
					},5000);
					//octopus.getResult();
					//$('#output').show();
